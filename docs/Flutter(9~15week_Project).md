...
### ✅ 이번 주 개발 목표
- 각 기능 화면별 기본 레이아웃 구현
- BottomNavigationBar를 통한 탭 구조 완성
...
- 하단 탭 네비게이션 (`BottomNavigationBar`) 구성 완료  
  → 홈 / 타이머 / 기록 / 통계 4개 탭 연결  
- 모든 화면을 `lib/screens/`로 분리하여 코드 관리 용이성 향상  
- `main.dart`는 `MainTabScreen()`을 루트 진입점으로 설정
...
### 💡 다음 주 계획 (12주차 예고)
- `SharedPreferences`를 활용한 집중 세션 로컬 저장 기능 구현
- 홈/기록 화면에서 실제 데이터 반영
- 타이머 종료 시 세션 자동 저장 로직 추가
